...
class CoolShipping_CoolRunner_Adminhtml_GridController extends Mage_Adminhtml_Controller_Action
{
			...
			if(isset($response['result']['shipments']) && is_array($response['result']['shipments'])){
				...
				foreach($response['result']['shipments'] as $shipment)
				{
					...
					if($order->getId())
					{
						...
						$table = $resource->getTableName('cm_coolrunner_sales_order_pdf');
